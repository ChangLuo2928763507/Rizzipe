...
// Update the user's record with the reset code and expiry
$stmt = $conn->prepare("UPDATE users SET reset_code = ?, reset_expiry = ? WHERE email = ?");
$stmt->bind_param("sis", $code, $expiry, $email);
$stmt->execute();

// Send the code via email (using PHP's mail function; in production, consider a more robust solution)
$subject = "Password Reset Confirmation Code";
$message = wordwrap("Your password reset code is: " . $code,70);
$headers = "From: cookcraft.noreply@gmail.com\r\n";

mail($email, $subject, $message, $headers);

// Return success response
echo json_encode(["status" => "success", "message" => "Confirmation code sent.","email" => $email]);
?>
